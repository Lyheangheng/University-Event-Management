 ...
 * Keeps same validation rules (5MB max size, allowed image MIMEs, random UUID filenames).
 */
@Injectable()
export class S3StorageProvider implements StorageProvider {
  ...
  constructor(private readonly configService: ConfigService) {
    this.bucketName = this.configService.get<string>('S3_BUCKET') || 'university-event-proofs';
  ...
  async save(file: StorageFile, options?: SaveFileOptions): Promise<StoredFileResult> {
    ...
    this.logger.log(`[S3StorageProvider] Saved object '${objectKey}' to bucket '${this.bucketName}' (${file.buffer.length} bytes).`);
  ...
  async getFilePath(filename: string, subfolder = 'proofs'): Promise<{ filePath: string; mimetype: string }> {
    ...
    return { filePath: objectKey, mimetype };
  }

  async delete(filename: string, subfolder = 'proofs'): Promise<boolean> {
    try {
      const { filePath: objectKey } = await this.getFilePath(filename, subfolder);
      this.logger.log(`[S3StorageProvider] Deleted object '${objectKey}' from bucket '${this.bucketName}'.`);
      return true;
    } catch {
      return false;
    }
